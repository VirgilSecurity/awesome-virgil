########
Private Keys Service API
########

* `General statements`_
* `Request encryption`_
* `Response decryption`_
* `Private Key`_
    * `POST /private-key`_
    * `POST /private-key/actions/grab`_
    * `POST /private-key/actions/delete`_
* `Appendix A. Responses`_
* `Appendix B. X-VIRGIL-REQUEST-SIGN`_
* `Appendix C. X-VIRGIL-REQUEST-ID`_
* `Appendix D. X-VIRGIL-ACCESS-TOKEN`_
* `Appendix E. response_password`_
...
* **RECEPIENT_ID** - Virgil Card ID of the Private Keys Service retrived from the Public Keys Service.
* **PUBLIC_KEY** - public key for the Private Keys Service retrived from the Public Keys Service.
...
You should do all these steps and provide a secure base64 encoded request for any of Private Keys Service endpoints in case you are using direct calls to the Private Keys Service. If Virgil PHP SDK is used, these steps will be produced in the background of the SDK. The end result of this algoritm will be a base64 encoded string:
...
You should do all these steps to decrypt an encrypted response in case you are using direct calls to the Private Keys Service. If Virgil PHP SDK is used, these steps will be produced in the background of the SDK. The end result of this algoritm will be a decrypted response:

.. code::

  [parameter_1 => parameter_1_value, parameter_2 => parameter_2_value]

Further in the text response data will be presented in unencrypted form for a correct understanding of the output parameters.

==========
Private Key
==========

:term:`Private Key <Private Key>` entity endpoints.
...
.. note::
  ...
  ``X-VIRGIL-ACCESS-TOKEN`` - For more information please check `Appendix D. X-VIRGIL-ACCESS-TOKEN`_ section.

  ``X-VIRGIL-REQUEST-SIGN`` - For more information please check `Appendix B. X-VIRGIL-REQUEST-SIGN`_ section.

  ``X-VIRGIL-REQUEST-ID`` - Header parameter that holds a random uuid to guarantee the uniqueness of the request body. For more information please check `Appendix C. X-VIRGIL-REQUEST-ID`_ section.
...
Request body for Identity Type Email
...
Request body for Identity Type Custom
...
.. code:: html
  ...
  60040 - Identity Type not supported.
  60050 - Identity Type has wrong format. Appears only in case of Custom Identity.
  60060 - Identity Token incorrect. Appears only in case of Custom Identity.
  60070 - Identity Request has already used before for provided Identity data. Appears only in case of Custom Identity
...
Library sign process for the request data using user’s private key. For generating we are using `Virgil Crypto Lib PHP Wrapper <https://packagist.org/packages/virgil/crypto>`_. The **X-VIRGIL-REQUEST-SIGN** hash is calculated on client side according to these rules:
