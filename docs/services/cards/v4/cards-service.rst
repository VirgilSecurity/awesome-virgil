...
This byte representation will be persisted and is not supposed to be changed within the **Virgil Card's** lifetime. This
JSON representation will be used to calculate the **Virgil Card Fingerprint**.
...
|                    | be specified if ``info`` parameter is defined                                                                                               |
+--------------------+---------------------------------------------------------------------------------------------------------------------------------------------+
| ``signs``          | (required) Must always contain **Virgil Card** holder's sign and either application sign or **Virgil Identity** sign (or both).             |
|                    | More about `signs`_                                                                                                                         |
+--------------------+---------------------------------------------------------------------------------------------------------------------------------------------+

.. [#] In order to create a global **Virgil Card** it's necessary to delegate the card creation to the **Virgil Registration Authority** service.
...
.. [#] Application **Virgil Cards** are accessible only within the application in which they were created. 
...
The **Authorization** HTTP header is mandatory for API calls that related to application scope and is skipped for global
cards. The access token can be retrieved for each application on the `Virgil
